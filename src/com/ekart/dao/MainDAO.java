...
public class MainDAO {
	...
	public static void findAll() {
		...
		try {
			...
			while (resultSet.next()) {
				System.out.println("Roll No " + resultSet.getInt(1) + ", Name " + resultSet.getString(2));
			}
	...
	public static boolean login(String username, String password) {
		...
		String count=null;
		try {
			 queryString = "SELECT * FROM customer where customerEmail='" + username + "'";
			connection = getConnection();
			ptmt = connection.prepareStatement(queryString);
			
			tc.trackTrace(queryString);
			resultSet = ptmt.executeQuery();
			count=Integer.toString(resultSet.getFetchSize());
			tc.trackTrace("count of records is " + count);
	...
	public static Category getCategories() {
		...
		try {
			...
			resultSet = ptmt.executeQuery();
			while (resultSet.next()) {
				category.getCategories().add(resultSet.getString("categoryName"));
			}
		} catch (SQLException e) {
			TelemetryClient tc=new TelemetryClient();
			tc.trackException(e);
	...
	public static boolean register(String name, String email, String phone, String state, String city, String zip,
			String password) {
		try {
	...
	public static Customer findCustomer(String email) {
		...
		try {
			String queryString = "SELECT * FROM customer where customerEmail='" + email + "'";
			connection = getConnection();
			ptmt = connection.prepareStatement(queryString);
			resultSet = ptmt.executeQuery();
			while (resultSet.next()) {
	...
	public static boolean update(String name, String email, String phone, String state, String city, String zip,
			String password) {
		try {
